...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (userRole !== 'admin') {
        setErrorMsg('Akses ditolak: Hanya admin yang dapat login di halaman ini.');
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0e0e0e] px-4">
      <div className="bg-gradient-to-br from-[#1a1a1a] via-[#1f1f1f] to-[#111] border border-gray-700 shadow-2xl rounded-2xl px-8 py-10 w-full max-w-md space-y-6">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-400">Masuk ke panel admin</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            <label className="block text-sm text-gray-300 mb-1">Email Admin</label>
            <input
              type="email"
              value={email}
          ...
          <div>
            <label className="block text-sm text-gray-300 mb-1">Password Admin</label>
            <input
              type="password"
              value={password}
