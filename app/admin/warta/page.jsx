...
export default function WartaPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditing) {
      await supabase.from("warta").update({ judul, isi }).eq('id', editId);
      alert("Warta berhasil diperbarui!");
    } else {
      await supabase.from("warta").insert({ judul, isi });
    ...
    <div className="p-4 sm:p-6 text-white bg-[#111] min-h-screen">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          title={showForm ? 'Tutup Form' : 'Tambah Warta'}
        >
      ...
      {showForm && (
        <div className="bg-[#1a1a1a] border border-gray-700 p-4 rounded-xl mb-8 shadow-sm">
          <h3 className="text-lg font-medium text-orange-400 mb-3">
            {isEditing ? 'Edit Warta' : 'Form Tambah Warta'}
          </h3>
      ...
      <div className="overflow-x-auto rounded-lg shadow-sm">
        <table className="min-w-full text-sm bg-[#1a1a1a] border border-gray-700">
          ...
          <tbody>
            {data.length === 0 && (
              <tr>
                <td colSpan="3" className="text-center text-gray-400 py-4">Belum ada warta.</td>
              </tr>
            )}
            {data.map((item, index) => (
              <tr key={item.id} className={index % 2 === 0 ? 'bg-[#1a1a1a]' : 'bg-[#181818]'}>
                ...
                <td className="px-4 py-3 border border-gray-700 text-center">
                  <div className="flex justify-center gap-2">
                    ...
                    <button onClick={() => deleteWarta(item.id)} title="Hapus" className="text-red-500 hover:text-red-400">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </td>
